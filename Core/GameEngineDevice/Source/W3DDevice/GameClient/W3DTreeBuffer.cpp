...
void W3DTreeBuffer::freeTreeBuffers()
{
	...
	if (m_dwTreePixelShader)
		DX8Wrapper::Delete_DX8_Pixel_Shader(m_dwTreePixelShader);
	m_dwTreePixelShader = 0;

	if (m_dwTreeVertexShader)
		DX8Wrapper::Delete_DX8_Vertex_Shader(m_dwTreeVertexShader);
	m_dwTreeVertexShader = 0;
}
...
//#define DEBUG_TEXTURE 1
	...
	if (m_dwTreeVertexShader) {
		...
		DX8Wrapper::Set_DX8_Vertex_Shader_Constant_Uncached(  4, &mat,  4 );
		Vector4 noSway(0,0,0,0);
		DX8Wrapper::Set_DX8_Vertex_Shader_Constant_Uncached(  8, &noSway,  1 );

		// c8 - c8+MAX_SWAY_TYPES - the sway amount.
		for	(i=0; i<MAX_SWAY_TYPES; i++) {
			Vector4 sway4(swayFactor[i].X, swayFactor[i].Y, swayFactor[i].Z, 0);
			DX8Wrapper::Set_DX8_Vertex_Shader_Constant_Uncached(  9+i, &sway4,  1 );
		}
		...
		if ((shroud=TheTerrainRenderObject->getShroud()) != nullptr) {
			...
			DX8Wrapper::Set_DX8_Vertex_Shader_Constant_Uncached(  32, &offset,  1 );
			width = 1.0f/(width*shroud->getTextureWidth());
			height = 1.0f/(height*shroud->getTextureHeight());
			offset.Set(width, height, 1, 1);
			DX8Wrapper::Set_DX8_Vertex_Shader_Constant_Uncached(  33, &offset,  1 );
		...
		if(TheGlobalData && TheGlobalData->m_useOverbright)
			mulTwoX = 1.0f;
		DX8Wrapper::Set_DX8_Pixel_Shader_Constant_Uncached(1, D3DXVECTOR4(mulTwoX, mulTwoX, mulTwoX, mulTwoX), 1);
#endif
	...
	for (bNdx=0;bNdx<MAX_BUFFERS; bNdx++) {
		...
		DX8Wrapper::Apply_Render_State_Changes();
		if (m_dwTreeVertexShader) {
			DX8Wrapper::Set_DX8_Vertex_Shader_Uncached(m_dwTreeVertexShader);

...
void W3DVolumetricShadow::RenderMeshVolume(Int meshIndex, Int lightIndex, const Matrix3D *meshXform)
{
	...
	DX8Wrapper::Set_DX8_Transform_Uncached(D3DTS_WORLD,&dxmWorld);
...
void W3DVolumetricShadow::RenderMeshVolumeBounds(Int meshIndex, Int lightIndex, const Matrix3D *meshXform)
{
	...
	DX8Wrapper::Set_DX8_Stream_Source(0,shadowVertexBufferD3D,sizeof(SHADOW_DYNAMIC_VOLUME_VERTEX));
	DX8Wrapper::Set_DX8_Vertex_Shader_Uncached(SHADOW_DYNAMIC_VOLUME_FVF);
...
void W3DVolumetricShadowManager::renderStencilShadows()
{
	...
	DX8Wrapper::Set_DX8_Vertex_Shader_Uncached(D3DFVF_XYZRHW | D3DFVF_DIFFUSE);
...
//  DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA );
		DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_SRCBLEND,  D3DBLEND_DESTCOLOR);
		DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_DESTBLEND, D3DBLEND_ZERO );


	// Set stencil states
    DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_ZENABLE,          TRUE );
		DX8Wrapper::Set_DX8_Render_State_Uncached(D3DRS_ZFUNC, D3DCMP_ALWAYS);

	// Only write where stencil val >= 1 (count indicates # of shadows that
	// overlap that pixel)
    DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_STENCILENABLE, TRUE );
    DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_STENCILFUNC, D3DCMP_LESSEQUAL );	//reference value is less or equal to stencil
    DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_STENCILPASS, D3DSTENCILOP_KEEP );
	...
	DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_STENCILMASK,     ~TheW3DShadowManager->getStencilShadowMask());
    DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_STENCILREF,      0x1 );


	DX8Wrapper::Set_DX8_Render_State_Uncached(D3DRS_SHADEMODE, D3DSHADE_FLAT);

	if (DX8Wrapper::_Is_Triangle_Draw_Enabled())
		DX8Wrapper::Draw_DX8_Primitive_UP(D3DPT_TRIANGLESTRIP, 2, v, sizeof(_TRANSLITVERTEX));

	DX8Wrapper::Set_DX8_Render_State_Uncached(D3DRS_SHADEMODE, D3DSHADE_GOURAUD);
	DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_ALPHABLENDENABLE, FALSE );
	// turn off the stencil buffer
	DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_STENCILENABLE, FALSE );
...
void W3DVolumetricShadowManager::renderShadows( Bool forceStencilFill )
{
	...
	if (m_shadowList && TheGlobalData->m_useShadowVolumes)
	{
	  ...
	  DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_ZENABLE,          TRUE );
		...
		DX8Wrapper::Set_DX8_Render_State_Uncached(D3DRS_ALPHATESTENABLE, FALSE);
		DX8Wrapper::Set_DX8_Render_State_Uncached(D3DRS_FOGENABLE, FALSE);


		// setup the TMU to default
		DX8Wrapper::Set_DX8_Render_State_Uncached(D3DRS_SHADEMODE, D3DSHADE_FLAT);
		DX8Wrapper::Set_DX8_Render_State_Uncached(D3DRS_LIGHTING, FALSE);
		...
		DX8Wrapper::Set_DX8_Texture_Stage_State_Uncached( 0, D3DTSS_COLOROP,   D3DTOP_SELECTARG2);
		...
		DX8Wrapper::Set_DX8_Render_State_Uncached(D3DRS_ALPHABLENDENABLE , TRUE);
		DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_STENCILENABLE, FALSE );
		DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_SRCBLEND, /*D3DBLEND_DESTCOLOR*/D3DBLEND_ONE );
		DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_DESTBLEND, D3DBLEND_ZERO );
		DX8Wrapper::Set_DX8_Render_State_Uncached(D3DRS_ZFUNC, D3DCMP_LESSEQUAL);
	#else
		//disable writes to color buffer
		if (DX8Wrapper::Get_Current_Caps()->Get_DX8_Caps().PrimitiveMiscCaps & D3DPMISCCAPS_COLORWRITEENABLE)
		{	oldColorWriteEnable=DX8Wrapper::Get_DX8_Render_State_Uncached(D3DRS_COLORWRITEENABLE);
			DX8Wrapper::Set_DX8_Render_State(D3DRS_COLORWRITEENABLE,0);
		}
		else
		{	//device does not support disabling writes to color buffer so fake it through alpha blending
			DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_SRCBLEND, D3DBLEND_ZERO );
			DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_DESTBLEND, D3DBLEND_ONE );
			DX8Wrapper::Set_DX8_Render_State_Uncached(D3DRS_ALPHABLENDENABLE , TRUE);
		}
		DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_STENCILENABLE, TRUE );
	#endif
		...
		if (TheW3DShadowManager->getStencilShadowMask() == 0x80808080)
			DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_STENCILFUNC,     D3DCMP_NOTEQUAL );	//in this mode, MSB indicates occluded player pixels.
		else
			DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_STENCILFUNC,     D3DCMP_GREATEREQUAL );	//in this mode, multiple bits indicate occluded player pixels.
		DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_STENCILREF,      0x80808080 );			//isolate MSB, it's used to indicate pixels containing potential occluders.
		DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_STENCILMASK,     TheW3DShadowManager->getStencilShadowMask());	//isolate upper bits containing PotentialOccluderBit|PlayerColorBits
		DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_STENCILWRITEMASK,0xffffffff );
		DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_STENCILZFAIL, D3DSTENCILOP_KEEP );
		DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_STENCILFAIL,  D3DSTENCILOP_KEEP );
		DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_STENCILPASS,  D3DSTENCILOP_INCR );

		DX8Wrapper::Set_DX8_Vertex_Shader_Uncached(SHADOW_DYNAMIC_VOLUME_FVF);

		DX8Wrapper::Set_DX8_Render_State_Uncached(D3DRS_CULLMODE,D3DCULL_CW);
//		DX8Wrapper::Set_DX8_Render_State_Uncached(D3DRS_ZBIAS,1);	///@todo: See if this helps or makes things worse.
		...
		DX8Wrapper::Set_DX8_Vertex_Shader_Uncached(W3DBufferManager::getDX8Format(W3DBufferManager::VBM_FVF_XYZ));
		...
		DX8Wrapper::Set_DX8_Render_State_Uncached( D3DRS_STENCILPASS,  D3DSTENCILOP_DECRSAT);
		...
		DX8Wrapper::Set_DX8_Render_State_Uncached(D3DRS_CULLMODE,D3DCULL_CCW);
		...
		DX8Wrapper::Set_DX8_Render_State_Uncached(D3DRS_CULLMODE,D3DCULL_CW);
//		DX8Wrapper::Set_DX8_Render_State_Uncached(D3DRS_ZBIAS,0);	///@todo: See if this helps or makes things worse.

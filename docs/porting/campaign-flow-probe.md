...
| Movies / cinematics reach FFmpeg | STATIC: USA-01's scripts contain **no** `PLAY_MOVIE`/`PLAY_MOVIE_FULLSCREEN`/`PLAY_MOVIE_RADAR` action; 0 video actions executed; `Display::playMovie`, `InGameUI::playMovie`, `FFmpegVideoPlayer::open` never hit. The mission's `IntroMovie` (`Mission::m_movieLabel`) is played by `LoadScreen.cpp` on the *campaign* route, which `-file` bypasses. **Movie audio** (Wave 14.3, `sound-effects-chain.md` §9, `ci-baselines/movie-audio-linux.json`): pre-fix the decoder found the intro's audio stream and decoded 80 frames of `EA_LOGO.BIK` that reached **no sink** (`onFrame` had audio only under `RTS_USE_OPENAL`, never defined here); fixed by feeding the `getHandleForBink` sample through `AIL_load_sample_buffer`. Rendered PCM matches `/usr/bin/ffmpeg` (correlation 1.000000, residual 0.016 % after the speech gain), A/V within one frame, handle released, music plays after | UNMEASURED (campaign route) — decoder proven in #102/#106; **movie audio MEASURED, Linux / rendered PCM / retail input** via `native_movie_audio_run`; Mac audibility UNMEASURED |

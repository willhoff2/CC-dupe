...
| `ci/replay-gate-manual` | `.github/workflows/check-replays.yml`, `scripts/ci/run-replays-local.ps1`, `docs/porting/replay-check-gamedata.md`, the replay section of `TESTING.md` | the replay gate skips with a stated reason instead of failing red when no game data is configured, and the manual run is a committed script rather than prose | this PR |

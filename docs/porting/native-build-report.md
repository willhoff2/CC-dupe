...
| `Core/GameEngineDevice` | 39 | 70 | 39 |
| `GeneralsMD/Code/GameEngineDevice` | 20 | 39 | 20 |
| `GeneralsMD/Code/Main` | 0 | 2 | 0 |
| **Total** | **777** | **837** | **777** |

60 translation units produced no object file:
...
| `Core/GameEngineDevice/Source/W3DDevice/GameClient/W3DSmudge.cpp` | `use of undeclared identifier 'TheD3D8RenderBackend'` |
...
**0 translation units that the probe calls clean fail to compile**, out of 777 probe-clean units (0%). These are the codegen-class failures `-fsyntax-only` cannot see.

## 3. Undefined symbols

The 9 archives were linked into one binary with `--whole-archive`, plus the third-party libraries the engine calls into: `libthirdparty_lzhl`, `z (system)` (binary produced: no; clean link: no). **352 distinct symbols are unresolved** once libc, libstdc++, libm, libpthread and the CRT/unwinder symbols are discounted. The full categorised list is in the JSON output; examples follow each count.

| Cause | Symbols |
|---|---:|
| Well-known Dict keys: `Core/GameEngineDevice/Source/W3DDevice/GameClient/WorldHeightMap.cpp` failed to compile | 104 |
| Defined in a translation unit that failed to compile | 103 |
| Defined in a layer not built here (renderer / audio) | 84 |

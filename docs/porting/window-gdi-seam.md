...
`scripts/native-build.py --with-shims --strict-link`, against `main` `ee059ee3d`. This slice also
landed the D3DX texture/FVF/shader entry points (`docs/porting/d3dx8-texture-seam.md`), so the D3DX
column moves here too.

| | before | after |
|---|---:|---:|
| Levels 1-4: object files | 969 / 972 | **973 / 976** |
| Levels 1-4: unresolved symbols | 173 | **168** |
...
| Levels 1-3: unresolved symbols | 548 | **556** |
...
levels 1-4 and rises by 8 at levels 1-3.** `platform_win32_user.cpp` now references twelve seam
...
work.** They are `D3DXFilterTexture` and `D3DXAssembleShader`, both defined in `WW3D2` — a level-4
library. `native-build.py` matches the `D3DX` pattern before it consults its "defined in a layer not
built here" evidence, so at a level set that does not build `WW3D2` they land in the port pile rather
than the harness one. At levels 1-4, where `WW3D2` is built, both resolve. `scripts/native-build.py`
is slice 4's file (the harness), so this is reported rather than fixed here.
...
Windows code is `WWAudio/listenerhandle.h` — see §6 — so the Windows build and the replay gate were
run for that.

...
// DirectInput keyboard scancodes. These are PS/2 scan code set 1, which is what
// `GameClient/KeyDefs.h` maps the engine's own key enum onto; the values matter because the
// engine persists them in keyboard config files.
...
// Aliases the real dinput.h also provides.

#!/usr/bin/env python3
"""Build and run the behaviour test for the user32 cursor and window-state seam.

GetCursorPos(), ScreenToClient(), SetCursor() and IsIconic() are defined off Windows in
...
the system pointer or shows it, or whether the coordinates arrive unscaled -- and W3DMouse::draw()
draws the game's cursor at the result of the first two, so a sign error moves the cursor and a
missed hide leaves two of them on screen. None of that is visible in a link, and the points/pixels
half of it is not visible on the CI runner either, whose backing scale is 1 where a Retina display's
is 2. See docs/porting/decisions-resolved.md.
...
Usage:
    python3 scripts/native-win32-cursor-test.py [--keep] [--verbose]
...
SOURCES = [
    ...
    f"{PLATFORM_DIR}/tests/win32_cursor_test.cpp",
]
...
def main():
    ...
    build_dir = pathlib.Path(tempfile.mkdtemp(prefix="win32-cursor-test-"))
    try:
        binary = build_dir / "win32_cursor_test"
        ...
        if run([str(binary)], args.verbose).returncode != 0:
            print("FAILED: the cursor and window-state seam does not behave as the call sites "
                  "expect")
            return 1

        print("OK: the user32 cursor and window-state seam behaves as the call sites expect")
        return 0
    finally:
        if args.keep:

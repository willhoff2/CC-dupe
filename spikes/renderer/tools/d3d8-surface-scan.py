        ...
        for i, line in enumerate(text.splitlines(), 1):
            ...
            for obj, method in CALL_RE.findall(line):
                if obj in dev_names and method in DEVICE8_METHODS:
                    dev[method] += 1
                    sites[method].append((rel, i, "direct"))
                    per_file[rel] += 1
                elif obj in d3d_names and method in D3D8_METHODS:
                    d3d[method] += 1
                    sites["IDirect3D8::" + method].append((rel, i, "direct"))
                    per_file[rel] += 1
...
print(f"IDirect3DDevice8: {len(dev)} distinct methods, {dev_total} call sites "
      f"({dev_macro_total} via DX8CALL/_HRES, {dev_total - dev_macro_total} direct)")
print(f"IDirect3D8:       {len(d3d)} distinct methods, {d3d_total} call sites "
      f"({d3d_macro_total} via DX8CALL_D3D, {d3d_total - d3d_macro_total} direct)")
print(f"TOTAL:            {len(dev) + len(d3d)} distinct methods, {dev_total + d3d_total} call sites "
      f"({dev_macro_total + d3d_macro_total} macro, "
      f"{dev_total + d3d_total - dev_macro_total - d3d_macro_total} direct)")
print()
print("| IDirect3DDevice8 method | sites | DX8CALL | direct |")
print("| --- | --- | --- | --- |")
for k, v in sorted(dev.items(), key=lambda x: (-x[1], x[0])):
    print(f"| `{k}` | {v} | {dev_macro.get(k, 0)} | {v - dev_macro.get(k, 0)} |")
print()
print("| IDirect3D8 method | sites | DX8CALL_D3D | direct |")
print("| --- | --- | --- | --- |")
for k, v in sorted(d3d.items(), key=lambda x: (-x[1], x[0])):
    print(f"| `{k}` | {v} | {d3d_macro.get(k, 0)} | {v - d3d_macro.get(k, 0)} |")
print()
